...
const Checkbox = ({ label, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) => (
  <label className="flex items-center space-x-3 cursor-pointer">
    <input type="checkbox" {...props} className="h-5 w-5 rounded border-gray-300 text-sky-600 focus:ring-sky-500" />
    <span className="text-maz-text">{label}</span>
  </label>
);

const AddAdRoom: React.FC<AddAdRoomProps> = ({ onClose }) => {
  ...
  const handleImageSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    filesToProcess.forEach(file => {
      // FIX: Add a type guard to ensure `file` is a Blob before using it with FileReader.
      // This resolves a TypeScript error where `file` was being inferred as `unknown`
      // and could not be assigned to the `Blob` parameter of `readAsDataURL`.
  ...
  return (
    <motion.div
      className="fixed inset-0 bg-black bg-opacity-60 z-[999] flex flex-col justify-end"
    ...
    >
      ...
      >
        ...
        <main className="flex-grow overflow-y-auto p-6 space-y-8">
          ...
          <section>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-maz-text-secondary mb-1 block">Title</label>
                <Input type="text" placeholder="e.g., Vintage Leather Jacket" />
              </div>
              <div>
                <label className="text-sm font-medium text-maz-text-secondary mb-1 block">Description</label>
                <Textarea rows={4} placeholder="Describe your item in detail..."></Textarea>
              </div>
              ...
              <div>
                  <label className="text-sm font-medium text-maz-text-secondary mb-1 block">Location</label>
           ...
           {/* Extra Options */}
          <section>
            <h3 className="font-semibold mb-3 text-lg">Extra Options</h3>
            <div className="space-y-3">
              <Checkbox label="Make it a sponsored ad" />
              <Checkbox label="Boost on social media" />
              <Checkbox label="Allow bidding (turn into an auction)" />
            </div>
          </section>
        </main>
        ...
        <footer className="flex-shrink-0 p-4 bg-maz-surface border-t border-gray-200 dark:border-gray-700 flex items-center justify-end gap-4">
          ...
          <button className="px-6 py-3 rounded-lg font-semibold text-white bg-sky-500 hover:bg-sky-600 transition">
            Publish Ad
          </button>
        </footer>
      </motion.div>
