...
const PaidAdCard: React.FC<PaidAdCardProps> = ({ data, isActive, onClick, onShowDetails, onShare }) => {
  ...
  const changeImage = (newDirection: number, e: React.MouseEvent) => {
    ...
    if (newDirection > 0) {
      ...
      setCurrentImageIndex((prev) => (prev - 1 + data.imageUrls.length) % data.imageUrls.length);
    }
  };
  ...
  return (
    ...
    >
      <div className="relative w-full h-32 overflow-hidden ad-card-image-container">
        <div className="absolute top-2 left-2 bg-maz-secondary text-white text-xs font-bold px-2 py-1 rounded-md z-20">
          Sponsored
        </div>
      ...
      >
        ...
        <div className="flex items-center justify-between">
            ...
            <div
                ...
                aria-controls={`paid-ad-details-${data.id}`}
            >
                {isActive ? 'Less' : 'More'}
            </div>
        </div>
      ...
      {/* Expanded Content */}
      <div id={`paid-ad-details-${data.id}`} className={`overflow-hidden ${isActive ? 'max-h-96' : 'max-h-0'}`}>
        <div className="p-4 pt-0">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="flex-1 bg-maz-secondary text-white py-2 rounded-md font-semibold"
            >
              Learn More
            </button>
            <button
